...
def run_config(name, **kw):
    ...
    print(f"\n  final: {step} steps in {dt:.1f}s, loss={final_loss:.4f}")
    ...
    return True, dict(train_loss=final_loss, val_loss=val_loss, steps=step, params=nparams)
...
print(f"\n{'='*60}")
print("SUMMARY")
print(f"{'Config':40s}  {'Pass':4s}  {'Train':>8s}  {'Val':>8s}  {'Steps':>5s}  {'Params':>10s}")
for name, (ok, m) in results.items():
    if m:
        print(f"  {name:38s}  {'PASS' if ok else 'FAIL'}  {m['train_loss']:8.4f}  {m['val_loss']:8.4f}  {m['steps']:5d}  {m['params']:10,}")
    else:
